 ...
 */
	public function testFieldDoubleEscaping() {
		...
		$test->expects($this->at(0))->method('execute')
			->with('SELECT `Article`.`id` FROM ' . $test->fullTableName('patients') . ' AS `Article`   WHERE 1 = 1');
 ...
 */
	public function testExecute() {
		$query = 'SELECT * FROM ' . $this->Dbo->fullTableName('patients') . ' WHERE 1 = 1';
		$this->Dbo->took = null;
 ...
 */
	public function testUpdateStatements() {
		...
		$this->Dbo->expects($this->at(0))->method('execute')
			->with("UPDATE `$db`.`patients` SET `field1` = 'value1'  WHERE 1 = 1");

		$this->Dbo->expects($this->at(1))->method('execute')
			->with("UPDATE `$db`.`patients` AS `Article` LEFT JOIN `$db`.`users` AS `User` ON " .
 ...
 */
	public function testDeleteStatements() {
		...
		$this->Dbo->expects($this->at(0))->method('execute')
			->with("DELETE  FROM `$db`.`patients`  WHERE 1 = 1");

		$this->Dbo->expects($this->at(1))->method('execute')
			->with("DELETE `Article` FROM `$db`.`patients` AS `Article` LEFT JOIN `$db`.`users` AS `User` " .
 ...
 */
	public function testTruncateStatements() {
		...
		$this->Dbo->expects($this->at(0))->method('execute')
			->with("TRUNCATE TABLE `$schema`.`patients`");
		$this->Dbo->truncate($Article);

		$this->Dbo->expects($this->at(0))->method('execute')
			->with("TRUNCATE TABLE `$schema`.`patients`");
		$this->Dbo->truncate('patients');
