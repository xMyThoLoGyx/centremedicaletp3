 ...
 */
	public function testOldQuery() {
		...
		$query .= $this->db->fullTableName('patients');
		$query .= ' WHERE ' . $this->db->fullTableName('patients') . '.id IN (1,2)';
		...
		$query .= $this->db->fullTableName('patients');
		$query .= ' WHERE ' . $this->db->fullTableName('patients') . '.id = 1';
		...
		$query .= $this->db->fullTableName('patients');
		$query .= ' WHERE ' . $this->db->fullTableName('patients');
 ...
 */
	public function testPreparedQuery() {
		...
		$query .= $this->db->fullTableName('patients');
		$query .= ' WHERE ' . $this->db->fullTableName('patients');
		$query .= '.id = ? AND ' . $this->db->fullTableName('patients') . '.published = ?';

		$params = array(1, 'Y');
		$result = $Article->query($query, $params);
		$expected = array(
			'0' => array(
				$this->db->fullTableName('patients', false, false) => array(
					'title' => 'First Article', 'published' => 'Y')
		));

		if (isset($result[0][0])) {
			$expected[0][0] = $expected[0][$this->db->fullTableName('patients', false, false)];
			unset($expected[0][$this->db->fullTableName('patients', false, false)]);
		}
		...
		$query .= $this->db->fullTableName('patients');
		$query .= '  WHERE ' . $this->db->fullTableName('patients') . '.title = ?';

		$params = array('First Article');
		$result = $Article->query($query, $params, false);
		$this->assertTrue(is_array($result));
		$this->assertTrue(
			isset($result[0][$this->db->fullTableName('patients', false, false)]) ||
			isset($result[0][0])
		);
		$result = $this->db->getQueryCache($query, $params);
		...
		$query = 'SELECT title FROM ';
		$query .= $this->db->fullTableName('patients');
		$query .= ' WHERE ' . $this->db->fullTableName('patients') . '.title LIKE ?';

		$params = array('%First%');
		$result = $Article->query($query, $params);
		$this->assertTrue(is_array($result));
		$this->assertTrue(
			isset($result[0][$this->db->fullTableName('patients', false, false)]['title']) ||
			isset($result[0][0]['title'])
		);

		//related to ticket #5035
		$query = 'SELECT title FROM ';
		$query .= $this->db->fullTableName('patients') . ' WHERE title = ? AND published = ?';
 ...
 */
	public function testParameterMismatch() {
		...
		$query = 'SELECT * FROM ' . $this->db->fullTableName('patients');
		$query .= ' WHERE ' . $this->db->fullTableName('patients');
		$query .= '.published = ? AND ' . $this->db->fullTableName('patients') . '.user_id = ?';
		$params = array('Y');
 ...
 */
	public function testVeryStrangeUseCase() {
		...
		$param = array(
			$this->db->fullTableName('patients'),
			$this->db->fullTableName('patients') . '.user_id', '3',
			$this->db->fullTableName('patients') . '.published', 'Y'
		);
